...
function createUnavailableController(){ return {renderMarkers(){},focus(){},focusArea(){},highlight(){},moveTo(){},resize(){},locate(){return Promise.reject(new Error("지도를 사용할 수 없습니다."));}}; }
...
export async function createMapController(){
  try {
    ...
    function highlight(id,active=true){
      ...
      if(!record) return;
      record.marker.setZIndex(active?1000:0);
      if(active) openInfoWindow(record);
      else if(openedInfoWindow===record.infoWindow){record.infoWindow.close();openedInfoWindow=null;}
    }
    function moveTo(lat,lng,zoom=16){ if(!Number.isFinite(Number(lat))||!Number.isFinite(Number(lng))) return;map.setCenter(new maps.LatLng(lat,lng));map.setZoom(zoom); }
    ...
    function resize(){ maps.Event.trigger(map,"resize"); }
    return {renderMarkers,focus,focusArea,highlight,moveTo,resize,locate};
