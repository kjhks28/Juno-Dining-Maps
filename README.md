...
내가 다녀온 맛집과 가고 싶은 곳을 지도, 사진, 평점, 방문 기록, 편의 태그와 함께 관리하는 개인용 웹앱입니다. 시·도와 세부 행정구역, 카테고리, 태그별 필터를 제공하며 관리자 변경 사항은 Supabase 공용 데이터에 자동 저장됩니다.
...
4. Authentication의 URL Configuration에서 Site URL을 배포 주소로 설정하고 Redirect URLs에 로컬 및 배포 주소를 등록합니다.
5. 사이트에서 관리자 이메일로 Magic Link를 받아 로그인합니다. 이후 등록·수정·삭제·방문 기록은 Supabase 공용 목록에 자동 반영됩니다.
