...
const elements = {
  ...
  regions:document.querySelector("#regionFilters"), categories:document.querySelector("#categoryFilters"), statusFilters:document.querySelector("#statusFilters"),
...
function createChip(label,type,value=label){
  const selected=type==="region"?activeRegion:type==="category"?activeCategory:activeStatus; const button=document.createElement("button"); button.className=`chip ${selected===value?"active":""}`; button.textContent=label;
  button.addEventListener("click",()=>{ if(type==="region") activeRegion=value; else if(type==="category") activeCategory=value; else activeStatus=value; render(); }); return button;
}
function renderFilters(){
  const regions=["전체",...new Set(restaurants.map(item=>item.region))]; const categories=["전체",...new Set(restaurants.map(item=>item.category))];
...
async function importBackup(event){
  ...
  try { const parsed=JSON.parse(await file.text()); const records=Array.isArray(parsed)?parsed:parsed.restaurants; if(!Array.isArray(records)||!records.every(validateImportedItem)) throw new Error("형식 오류"); if(!window.confirm(`현재 기록을 백업의 ${records.length}개 기록으로 교체할까요?`)) return; restaurants=records.map(normalizeRestaurant); activeStatus="all";activeRegion="전체";activeCategory="전체";activeId=null;saveRestaurants(restaurants);render();window.alert("백업을 성공적으로 불러왔어요."); }
